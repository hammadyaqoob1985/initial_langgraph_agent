...
info_parse_prompt = ChatPromptTemplate.from_messages(
    [
        (
            ...
            maximum potential fine from the message. If there is no maximum 
            potential fine in the message then set that field to 0. 
            For any other fields if any of the fields
            aren't present, don't populate them. Try to cast dates into
            the YYYY-mm-dd format. Don't populate fields if they're not
            present in the message unless it is the max potential fine field.
